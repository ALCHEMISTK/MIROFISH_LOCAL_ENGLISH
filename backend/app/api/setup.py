...
logger = get_logger('mirofish.api.setup')
        ...
        for key in ['llm_api_key', 'llm_base_url', 'llm_model_name',
            ...
            if key in data:
                if not isinstance(data[key], str):
                    return jsonify({'success': False, 'error': f'{key} must be a string'}), 400
    ...
    try:
        ...
        try:
            ...
            if 'api_key' in error_msg.lower() or 'apikey' in error_msg.lower() or 'api-key' in error_msg.lower():
                results['errors'].append("LLM validation failed: authentication error")
            else:
