...
from ..services.lightrag_entity_reader import ZepEntityReader
...
def prepare_simulation():
    ...
    try:
        ...
        try:
            ...
            # Save entity count to state (for frontend to immediately access)
...
def get_prepare_status():
    ...
    try:
        ...
        if not task:
            # Task does not exist, but if simulation_id is provided, check if preparation is already complete
            if simulation_id:
                is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
                if is_prepared:
                    return jsonify({
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    and returns the latest one (sorted by created_at)
...
def download_simulation_script(script_name: str):
    """
    Download simulation run script file (common scripts in backend/scripts/)
...
def start_simulation():
    ...
    Request (JSON):
        {
            ...
            "max_rounds": 100,                     // optional: maximum simulation rounds, for truncating overly long simulations
    ...
    About force parameter:
        - When enabled, if simulation is running or completed, it will be stopped first and run logs cleared
...
def get_run_status_detail(simulation_id: str):
    ...
    try:
        ...
        # recent_actions only shows the current latest round for both platforms
...
def interview_agent():
    ...
    Note: This feature requires the simulation environment to be in running state (waiting for commands mode after completing simulation loop)
...
def get_env_status():
    ...
    Checks if the simulation environment is alive (can receive Interview commands)
...
def close_simulation_env():
    ...
    Sends a close environment command to the simulation for graceful exit from waiting-for-commands mode.
