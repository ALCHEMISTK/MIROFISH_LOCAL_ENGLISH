...
Achieves full automation without manual parameter tuning.

Adopts a step-by-step generation strategy to avoid failures from generating
overly long content in one shot:
...
from .lightrag_entity_reader import EntityNode, ZepEntityReader
...
CHINA_TIMEZONE_CONFIG = {
    ...
    "activity_multipliers": {
        "dead": 0.05,      # Almost no one active late night
...
class AgentActivityConfig:
    ...
    # Response speed (delay to trending events, in simulated minutes)
...
class TimeSimulationConfig:
    ...
    off_peak_activity_multiplier: float = 0.05  # Extremely low activity late night
...
class PlatformConfig:
    ...
    # Viral spread threshold (interactions required to trigger spread)
...
class SimulationConfigGenerator:
    ...
    EVENT_CONFIG_CONTEXT_LENGTH = 5000   # Event configuration
    ENTITY_SUMMARY_LENGTH = 200          # Entity summary
    AGENT_SUMMARY_LENGTH = 200           # Entity summary in agent config
    ...
    def _build_context(
        ...
        remaining_length = self.MAX_CONTEXT_LENGTH - current_length - 500  # Keep 500 char margin
...
- User base is Chinese; schedule must conform to Beijing time habits
...
- Evening peak 19-22 (activity multiplier 1.5)
- Activity declines after 23:00 (activity multiplier 0.5)
- General pattern: low activity late night, increasing in morning, moderate during work hours, peak in evening
- **Important**: The values below are examples only; adjust based on event type and participant group:
  - e.g., Student groups may peak at 21-23; media is active all day; official institutions only during work hours
  - e.g., Breaking news may generate late-night discussion, so off_peak_hours can be shortened
...
Field descriptions:
- total_simulation_hours (int): Total simulation duration, 24-168 hours; short for breaking events, longer for ongoing topics
- minutes_per_round (int): Round duration, 30-120 minutes, recommend 60
...
**Important**: poster_type must be selected from the "Available entity types" above, so that initial posts can be assigned to the correct Agent for posting.
...
{{
    ...
    def _assign_initial_post_agents(
        ...
        # Track used agent indices per type to avoid assigning the same agent repeatedly
...
- **Individuals** (Student/Person/Alumni): High activity (0.6-0.9), mostly active in evening (18-23), fast response (1-15 min), low influence (0.8-1.2)
...
{{
    "agent_configs": [
        {{
            ...
            "active_hours": [<list of active hours, considering Chinese schedule>],
    ...
    def _generate_agent_config_by_rule(self, entity: EntityNode) -> Dict[str, Any]:
        ...
        if entity_type in ["university", "governmentagency", "ngo"]:
            ...
            # Experts/professors: active during work and evening hours, moderate frequency
            return {
                "activity_level": 0.4,
                "posts_per_hour": 0.3,
            ...
            # Students: mainly active in evening, high frequency
            return {
                "activity_level": 0.8,
                "posts_per_hour": 0.6,
            ...
            # Alumni: mainly active in evening
            return {
                "activity_level": 0.6,
                "posts_per_hour": 0.4,
