...
- We need to simulate the reactions of various parties in a public opinion event and information dissemination paths

Therefore, **entities must be real, existing subjects in the real world that can post and interact on social media**:
...
A. **Fallback types (required, placed at the end of the list as the last 2)**:
   - `Person`: Fallback type for any individual person. When a person does not fit any other more specific person type, they go here.
   - `Organization`: Fallback type for any organization. When an organization does not fit any other more specific organization type, it goes here.
...
- Text will contain various people, such as "primary school teachers", "passersby", "some netizen"
...
- Descriptions must clearly explain how this type differs from the fallback types
...
class OntologyGenerator:
    ...
    def _build_user_message(
        ...
        # If text exceeds 50,000 characters, truncate (only affects what is sent to LLM, not graph building)
...
4. All entity types must be real-world subjects that can post and speak; they cannot be abstract concepts
...
"""
    ...
    def _validate_and_process(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Final safety check to not exceed limits (defensive programming)
