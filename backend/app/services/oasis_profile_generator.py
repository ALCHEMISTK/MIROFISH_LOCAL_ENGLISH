...
class OasisProfileGenerator:
    ...
    # Group/institution type entities (require representative persona generation)
    GROUP_ENTITY_TYPES = [
    ...
    def generate_profile_from_entity(
        ...
        """
        Generate OASIS Agent Profile from a graph entity
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """Get system prompt"""
        base_prompt = "You are a social media user persona generation expert. Generate detailed, realistic personas for public opinion simulation, maximally reflecting real-world circumstances. You must return valid JSON format; all string values must not contain unescaped newlines. Use Chinese."
        return base_prompt
...
2. persona: Detailed persona description (2000-character plain text), including:
   ...
   - Positions and views (attitude toward topics, content that might provoke or move this person)
   - Unique traits (catchphrases, special experiences, personal hobbies)
   - Personal memory (important part of the persona: describe this individual's connection to the event, and their actions and reactions in the event)
...
2. persona: Detailed account profile description (2000-character plain text), including:
   - Basic institutional info (official name, nature, founding background, main functions)
   ...
   - Content publication characteristics (content types, publication frequency, active time periods)
   - Positions and attitudes (official stance on core topics, how to handle controversy)
   - Special notes (representative group profile, operational habits)
   - Institutional memory (important part of the institutional persona: describe this institution's connection to the event, and its actions and reactions in the event)
...
- Institutional account communications must match its identity and positioning"""
    ...
    def generate_profiles_from_entities(
        ...
        # Helper function for real-time profile saving
