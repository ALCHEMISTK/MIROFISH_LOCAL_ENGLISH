...
if sys.platform == 'win32':
    ...
    # Note: this ideally needs to be set at Python startup; setting it at runtime may not always take effect.
    ...
    def _utf8_open(file, mode='r', buffering=-1, encoding=None, errors=None,
        ...
        This fixes the issue where third-party libraries (like OASIS) read files
        without specifying an encoding.
        """
...
def disable_oasis_logging():
    ...
    OASIS logs are very noisy (logging every agent observation and action); we use our own action_logger instead.
    """
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: Target platform (optional)
                ...
                - None/unspecified: interview both platforms and return merged result

        Returns:
            True on success, False on failure
        """
        # If a specific platform is given, interview only that platform
...
def get_agent_names_from_config(config: Dict[str, Any]) -> Dict[int, str]:
    ...
    placeholder names like "Agent_0".
...
def fetch_new_actions_from_db(
    ...
    Args:
        ...
        last_rowid: The maximum rowid read last time (rowid is used instead of created_at
...
def _enrich_action_context(
    ...
    try:
        ...
        # Repost: add original post content and author
            ...
            if new_post_id:
                # The reposted post's original_post_id points to the original
                cursor.execute("""
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    When a boost LLM is configured, parallel simulation can use different API providers
    for each platform, increasing concurrency capacity.

    Args:
        config: Simulation config dictionary
        use_boost: Whether to use the boost LLM config (if available)
    """
    # Check if a boost config is available
    boost_api_key = os.environ.get("LLM_BOOST_API_KEY", "")
    boost_base_url = os.environ.get("LLM_BOOST_BASE_URL", "")
    boost_model = os.environ.get("LLM_BOOST_MODEL_NAME", "")
    has_boost_config = bool(boost_api_key)

    # Select which LLM to use based on the parameter and available config
...
def get_active_agents_for_round(
    ...
    """Decide which agents to activate for the current round based on time and config"""
...
async def run_twitter_simulation(
    ...
    # Get real agent name mapping from config (use entity_name rather than the default Agent_X)
    agent_names = get_agent_names_from_config(config)
    # Fall back to OASIS default names for agents not in config
    ...
    total_actions = 0
    last_rowid = 0  # Track the last processed rowid in the database (use rowid to avoid created_at format differences)
...
async def run_reddit_simulation(
    ...
    # Reddit uses the boost LLM config (falls back to general config if unavailable)
...
async def main():
    ...
    parser.add_argument(
        ...
        action='store_true',
        default=False,
        help='Close environment immediately after simulation completes, without entering command-waiting mode'
    )
    ...
    if args.max_rounds:
        ...
        if args.max_rounds < config_total_rounds:
            log_manager.info(f"  - Actual rounds to run: {args.max_rounds} (capped)")
    ...
    if wait_for_commands:
        log_manager.info("")
        log_manager.info("=" * 60)
        log_manager.info("Entering command-waiting mode — environment remains active")
        ...
        try:
            ...
            print(f"\nError during command processing: {e}")
...
def setup_signal_handlers(loop=None):
    ...
    Persistent simulation scenario: simulation does not exit after completion — it waits for interview commands.
    ...
    def signal_handler(signum, frame):
        ...
        if not _cleanup_done:
            _cleanup_done = True
            # Set the event to notify the asyncio loop to exit (letting it clean up first)
            if _shutdown_event:
                _shutdown_event.set()

        # Do not call sys.exit() directly — let the asyncio loop exit cleanly and free resources.
