...
let pendingState = null  // Record the target state to be applied
...
// Format date (display date part only)
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Update the target state to apply (record latest intent regardless of animation state)
        ...
        // If currently animating, only record the state and handle it after animation ends
        ...
        // Use a debounce delay for state switching to prevent rapid flickering.
        // Expanding uses a shorter delay (50ms); collapsing uses a longer delay (200ms) for stability.
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // Check if the pending state still needs to be applied (may have been overridden by subsequent scrolling)
          ...
          setTimeout(() => {
            ...
            // After animation ends, check if there is a new pending state to apply
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Delay slightly before applying to avoid switching too quickly
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
...
/* Fixed-spacing square grid using CSS background pattern */
.grid-pattern {
  position: absolute;
  top: 0;
  left: 0;
  ...
  /* Anchor to top-left so height changes only extend at the bottom */
