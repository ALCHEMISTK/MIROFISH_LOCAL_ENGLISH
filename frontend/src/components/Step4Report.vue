...
const toggleRawResult = (timestamp, event) => {
  ...
  // After DOM update, adjust scroll to keep button at the same position
...
const parseInterview = (text) => {
  ...
  try {
    ...
    const interviewBlocks = text.split(/#### Interview #\d+[:\s]/).slice(1)
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      // Extract bio
      const bioMatch = block.match(/_(?:Bio|Type):\s*([\s\S]*?)_\n/)
      if (bioMatch) {
      ...
      if (answerMatch) {
        ...
        // Platform fallback logic (compatible with old format: only one platform marker present)
        if (!twitterMatch && redditMatch) {
          // Only Reddit answer present; copy to default display only if it's not placeholder text
          ...
          // No platform markers at all (very old format), treat entire text as answer
          interview.twitterAnswer = answerText
        }
      }
      
      // Extract key quotes (compatible with multiple quote formats)
      const quotesMatch = block.match(/\*\*Key quote[s]?:\*\*\n([\s\S]*?)(?=\n---|\n####|$)/)
      if (quotesMatch) {
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Drift Query Sub-problem Analysis'),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // If no match found, fall back to digit+dot format
      if (matches.length === 0) {
        const numPattern = /(?:^|[\r\n]+)(\d+)\.\s+/g
...
// Current active step (used for top-level display)
...
const renderMarkdown = (content) => {
  ...
  // Clean <br> before paragraph opening tags that immediately follow block-level elements
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix numbering for non-consecutive ordered lists: when single-item <ol> elements are separated by paragraph content, keep incrementing the counter
...
const extractFinalContent = (response) => {
  ...
  if (response.length > 300 && (response.includes('**') || response.includes('>'))) {
    // Remove leading Thought: thinking section
...
.header-dot {
  ...
  box-shadow: 0 0 0 3px var(--c-shadow);
  margin-right: 10px;
  flex-shrink: 0;
...
@keyframes pulse-dot {
  ...
  50% {
    box-shadow: 0 0 0 5px var(--c-shadow);
  }
}
...
.generated-content :deep(.md-h2) { font-size: 20px; border-bottom: 1px solid var(--c-surface-3); padding-bottom: 8px; }
...
.right-panel {
  ...
  --wf-border: var(--c-border-light);
  --wf-divider: var(--c-surface-3);

  --wf-active-bg: var(--c-surface-2);
  --wf-active-border: var(--c-text);
  --wf-active-dot: var(--c-text);
  --wf-active-text: var(--c-text);

  --wf-done-bg: var(--c-surface-2);
  --wf-done-border: var(--c-border-light);
  --wf-done-dot: #10B981;

  --wf-muted-dot: var(--c-border);
  --wf-todo-text: var(--c-text-muted);
}
...
html:not(.dark) .metric-pill.pill--completed {
  background: #ECFDF5;
  border-color: #A7F3D0;
  color: #065F46;
}
...
html:not(.dark) .section-tag.completed {
  background: #ECFDF5;
...
html:not(.dark) .section-tag.completed svg {
  color: #059669;
}
...
html:not(.dark) .section-tag.completed .tag-num {
  color: #059669;
}
...
html:not(.dark) .meta-tag.final-answer {
  background: #D1FAE5;
  color: #059669;
}
...
html:not(.dark) .final-answer-hint {
  background: #ECFDF5;
  border: 1px solid #A7F3D0;
  color: #065F46;
}
...
html:not(.dark) :deep(.panorama-display .entity-type) {
  background: #DBEAFE;
}
...
html:not(.dark) :deep(.quick-search-display .edge-label) {
  background: #FFEDD5;
  color: #C2410C;
}
...
html:not(.dark) :deep(.quick-search-display .node-type) {
  background: #FFEDD5;
}
