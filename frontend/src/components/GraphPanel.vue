    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">Some content is still processing. Consider refreshing the graph later.</span>
...
const renderGraph = () => {
  ...
  // Track the index of the current edge being processed for each node pair
  const edgePairIndex = {}
  ...
  tempEdges.forEach(e => {
    ...
    if (totalCount > 1) {
      ...
      // The curvature range grows with the number of edges.
      const curvatureRange = Math.min(1.2, 0.6 + totalCount * 0.15)
      curvature = ((currentIndex / (totalCount - 1)) - 0.5) * curvatureRange * 2

      // If the edge direction is opposite to the normalized direction, flip the curvature
      // so that all edges are distributed in the same reference frame and do not overlap.
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      // Dynamically adjust distance based on the number of edges between this pair.
    ...
    // Add gravitational pull toward the center so isolated node clusters gather in the center area
  ...
  const getLinkPath = (d) => {
    ...
    if (d.isSelfLoop) {
      ...
      const y2 = sy + 4
      // Draw self-loop using an arc (sweep-flag=1 clockwise)
    ...
    // Compute the curve control point — dynamically adjusted based on edge count and distance
    ...
    .call(d3.drag()
      ...
      .on('drag', (event, d) => {
        ...
        if (!d._isDragging && distance > 3) {
          // First time a real drag is detected: restart simulation
          d._isDragging = true
...
.graph-panel {
  ...
  background-image: radial-gradient(var(--c-border-light) 1.5px, transparent 1.5px);
...
.panel-header {
  ...
  align-items: center;
  background: linear-gradient(to bottom, color-mix(in srgb, var(--c-bg) 95%, transparent), transparent);
  pointer-events: none;
}
...
.self-loop-header {
  ...
  align-items: center;
  gap: 8px;
  background: linear-gradient(135deg, #0d1f14 0%, #111a0d 100%);
  border: 1px solid #2e5a2e;
}

html:not(.dark) .self-loop-header {
